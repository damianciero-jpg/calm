...
-- Tables referenced by the app: profiles, children, sessions, notifications,
-- iep_goals, therapist_notes. The reset also includes generic goals and notes.
...
CREATE INDEX notes_child_id_idx ON public.notes(child_id);
...
CREATE POLICY sessions_parent_delete ON public.sessions
  FOR DELETE TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.children
...
CREATE POLICY goals_related_all ON public.goals
...
CREATE POLICY notes_related_all ON public.notes
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.children
      WHERE children.id = notes.child_id
        AND (children.parent_id = auth.uid() OR children.therapist_id = auth.uid())
    )
  )
  WITH CHECK (
...
CREATE POLICY iep_goals_related_all ON public.iep_goals
  FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.children
...
CREATE POLICY therapist_notes_related_all ON public.therapist_notes
  FOR ALL TO authenticated
  USING (
